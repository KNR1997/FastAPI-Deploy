...
class UserService(CRUDBase[User, UserCreate, UserUpdate]):
    ...
    async def create_user(self, obj_in: UserCreate) -> User:
        if await self.get_by_username(obj_in.username):
            raise_field_error("username", "User with this username already exists.")

        if await self.get_by_email(obj_in.email):
            raise_field_error("email", "User with this email already exists.")
    ...
    async def update_user(self, user_id: int, obj_in: UserUpdate) -> User:
        existing_user = await self.model.get(id=user_id)
        ...
        # obj_in.password = get_password_hash(password=obj_in.password)
        obj = await self.update(id=user_id, obj_in=obj_in)
        return obj
    ...
    async def me(self) -> Optional[User]:
        return await self.model.filter(username='admin').first()
